...
function createApp(options = {}) {
  ...
  async function handleAdmin(req, res, url, pathname) {
    // 打开 Dashboard 时自动取一次令牌：仅放行本机回环地址
    // （NAS 上通过局域网 IP 访问不会命中，避免管理权限被同网段任意主机拿到）
    if (pathname === '/admin/api/bootstrap') {
      ...
      if (local || ctx.cfg.settings.allowRemoteBootstrap === true) {
        ...
        sendJson(res, 403, {
          error: {
            message: '远程访问不允许自动下发令牌。请在 config/providers.json 的 settings 里设置 adminToken，或在页面手动填入管理令牌。',
