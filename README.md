...
# 3) 自检：50 项端到端断言，覆盖同意书门禁/鉴权/流式/故障转移/熔断/计量/推送/大屏
node scripts/selfcheck.js
#    （可选）再跑一遍大屏渲染冒烟：node scripts/ui-smoke.js   —— 57 项
...
产物就在项目根目录：**`free-model-gateway1.2.2.fpk`**（约 100 KB）
...
CI 会自动：跑端到端自检 + 大屏冒烟 → 打包 `.fpk` → 生成 `free-model-gateway-1.2.2.tgz` 与 `.sha256` → 发布 Release。
